...
<h3>
    @if($tipo == 1)
        FORMULARIO PARA EL REGISTRO DE PISTOLA DEL {{grado_nombre_completo($user_id)}}
    @endif
    @if($tipo == 2)
        FORMULARIO PARA EL REGISTRO DE CUCHILLO BAYONETA DEL {{grado_nombre_completo($user_id)}}
    @endif
</h3>
@endsection

@section('content')
    {!! Form::open(['route' => 'armamentos_users.store', 'method' => 'post']) !!}
        @csrf
        ...
        <div class="form-group" id="user_id" style="display: none">
            ...
            {{ Form::text('tipo', $tipo, ['class' => 'form-control', 'required', 'readonly']) }}
            @error('tipo')
            <small>* {{$message}}</small>
            <br>
            @enderror
        ...
        @if($tipo == 2)
            <div class="form-group">
                {{ Form::label('uso', 'USO') }}
                {{ Form::text('uso', null, ['placeholder' => 'CUCHILLO DE ASALTO', 'class' => 'form-control', 'style' => 'text-transform:uppercase', 'onkeyup' =>'javascript:this.value=this.value.toUpperCase()']) }}
                @error('uso')
                <small>* {{$message}}</small>
                <br>
                @enderror
        ...
        @if($tipo == 2)
            <div class="form-group">
                ...
                {{ Form::text('accesorios', null, ['placeholder' => 'SIN ACCESORIOS', 'class' => 'form-control', 'style' => 'text-transform:uppercase', 'onkeyup' =>'javascript:this.value=this.value.toUpperCase()']) }}
                @error('accesorios')
                <small>* {{$message}}</small>
                <br>
                @enderror
