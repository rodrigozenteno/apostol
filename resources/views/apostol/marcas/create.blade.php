...
<h3>Formulario para registro de marcas</h3>
@endsection
...
@section('content')
    {!! Form::open(['route' => 'marcas.store', 'method' => 'post']) !!}
        ...
        <div class="form-group">
            ...
            {{ Form::text('marca', null, ['placeholder' => 'TAURUS', 'class' => 'form-control', 'required', 'style' => 'text-transform:uppercase', 'onkeyup' =>'javascript:this.value=this.value.toUpperCase()']) }}
            @error('marca')
            <small>* {{$message}}</small>
            <br>
            @enderror
        ...
        <div class="form-group" id="industria_id">
            ...
            {{ Form::select('industria_id', $industrias, null, ['class' => 'form-control select-industria', 'multiple', 'required']) }}
            @error('industria_id')
            <small>* {{$message}}</small>
            <br>
            @enderror
